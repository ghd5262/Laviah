...
CItemShopPopup* CItemShopPopup::create()
{
    CItemShopPopup *pRet = new(std::nothrow) CItemShopPopup();
    if (pRet && pRet->init())
    {
        pRet->autorelease();
        return pRet;
...
bool CItemShopPopup::init()
{
    ...
    if (listView != nullptr){
        ...
        for (auto iter : itemList)
        {
            ...
            auto itemDP = CItemShopPopupDP::create(item);
...
void CItemShopPopup::End(Node* sender){
