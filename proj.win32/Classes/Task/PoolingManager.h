...
* 이전에 PoolingManager에서 모든 메모리 블럭을 처음부터 Shooter객체로 초기화하고 
* 생성자 호출 및 addChild를 하는 구조로 짰을때는 중간에 다른 Shooter로 초기화 및 사용할 수 없는
* 문제가 생겼다.
...
public:
	...
	void CreateShooterList(size_t count, size_t size);	// size만큼의 char형 포인터를 count만큼 Shooter리스트에 add
	void* BulletNew();									// pool이 보유한 메모리 리스트가 생성하려는 것보다 적으면 새로 생성
	void* ShooterNew();									// pool이 보유한 메모리 리스트가 생성하려는 것보다 적으면 새로 생성
	void Bullet_ReturnToFreeMemory(void* bullet);		// Bullet을 메모리블럭으로 전환 (alive off)
	void Shooter_ReturnToFreeMemory(void* shooter);		// Shooter를 메모리블럭으로 전환 (alive off)
	void Bullet_ReturnToFreeMemoryAll();				// 모든 Bullet을 메모리 블럭으로 전환 (alive off)
	void Shooter_ReturnToFreeMemoryAll();				// 모든 SHooter를 메모리 블럭으로 전환 (alive off)
