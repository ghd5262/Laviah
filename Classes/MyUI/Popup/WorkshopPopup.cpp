...
bool CWorkshopPopup::init()
{
	...
	if (bg != nullptr){
		...
		bg->setPosition(Vec2(layerSize.width * 0.5f, layerSize.height * 0.5f));
		this->addChild(bg);
	}
...
//	}
	...
	auto itemList       = CWorkshopItemDataManager::Instance()->getItemList();
    typedef std::pair<int, const WORKSHOPITEM_PARAM*> PAIR;
    auto sortedList     = std::vector<PAIR>(itemList.begin(), itemList.end());
    // sort
    {
        // sort by clear (clear achievements locate to the top.)
        std::sort(sortedList.begin(), sortedList.end(), [=](PAIR dataA, PAIR dataB){
            return dataA.second->_sortIndex < dataB.second->_sortIndex;
        });
    }
